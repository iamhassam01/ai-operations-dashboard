...
function ToolCallIndicators({ toolCalls }: { toolCalls: { tool: string; status: string }[] }) {
    ...
    <div className="mt-1.5">
      ...
      {expanded && (
        <div className="mt-1 space-y-0.5 ml-4">
          {toolCalls.map((tc, i) => {
            const ToolIcon = toolIcons[tc.tool] || Zap;
            return (
              <div key={i} className="flex items-center gap-2 text-[11px] text-[var(--text-tertiary)]">
                <ToolIcon size={12} />
                <span className="capitalize">{tc.tool.replace(/_/g, ' ')}</span>
                <span className={tc.status === 'success' ? 'text-success' : tc.status === 'error' ? 'text-error' : ''}>
...
function ResponseMetadata({ metadata }: { metadata: Message['metadata'] }) {
  if (!metadata) return null;
  const { response_time_ms, tokens_used, cost } = metadata;
  if (!response_time_ms && !tokens_used && !cost) return null;

  return (
    <div className="flex items-center gap-2 text-[10px] text-[var(--text-tertiary)] px-1 mt-0.5">
      {response_time_ms != null && <span>⏱ {(response_time_ms / 1000).toFixed(1)}s</span>}
      {tokens_used != null && <span>· {tokens_used} tokens</span>}
    ...
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} px-4 py-1.5`}>
      <div className={`flex items-start gap-2.5 max-w-[85%] md:max-w-[70%] ${isUser ? 'flex-row-reverse' : ''}`}>
        ...
        <div className={`space-y-1 ${isUser ? 'items-end' : 'items-start'}`}>
          <div className={`rounded-[var(--radius-lg)] px-3 py-2 sm:px-3.5 sm:py-2.5 text-sm leading-relaxed ${
            isUser
          ...
          {!isUser && message.metadata?.tool_calls && message.metadata.tool_calls.length > 0 && (
            <ToolCallIndicators toolCalls={message.metadata.tool_calls} />
          )}
          ...
          {/* Response metadata (assistant only) */}
          {!isUser && <ResponseMetadata metadata={message.metadata} />}
        </div>
      </div>
    </div>
  );
            ...
            >
              ...
              {confirmDeleteId === conv.id && (
                <div className="absolute inset-0 z-10 flex items-center justify-center gap-2 bg-[var(--status-error-surface)] border border-error/30 rounded-sm px-3">
                  ...
                  <button
                    ...
                    className="flex h-7 w-7 items-center justify-center rounded-[var(--radius-sm)] bg-error text-white hover:bg-error/90 transition-colors"
                  ...
                  <button
                    ...
                    className="flex h-7 w-7 items-center justify-center rounded-[var(--radius-sm)] bg-[var(--surface-primary)] text-[var(--text-secondary)] border border-[var(--border-default)] hover:bg-[var(--surface-hover)] transition-colors"
                    title="Cancel"
                  >
                    <X size={13} />
                  </button>
              ...
              {editingId === conv.id ? (
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <button onClick={saveEdit} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[var(--radius-sm)] text-success hover:bg-[var(--surface-hover)] transition-colors" title="Save">
                    <Check size={13} />
                  </button>
                  <button onClick={cancelEdit} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[var(--radius-sm)] text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] transition-colors" title="Cancel">
                    <X size={13} />
                  </button>
                </div>
              ) : (
